function clusVal = find_perm_clus_parallel(permSCI, smoothDims, clus_type, thresh_clus, c)

% FIND_PERM_CLUS_PARALLEL finds max cluster values for each permutation.
% Parallel implementation (requires Parallel Processing Toolbox). 
% See STATCI_MAX_PARALLEL function documentation for details.
%
...
for pred = 1:nElemNS
    ...
    parfor perm = 1:nPerm
        ...
        elseif n>1
            ...
            clusVal_temp(perm) = max(val);
        end
    end
    clusVal(pred,:) = clusVal_temp;
end
