...
if nargout>1 && all(abs(thresh_clus)<1)
    ...
    else % if too many elements, just base computation on a (big) sample)
        if strcmpi(tails,'left')
            thresh_clus = nanmean(quantile(permSCI(:,randperm(prod(nPred),100000)),thresh_clus));
        elseif strcmpi(tails,'right')
            thresh_clus = nanmean(quantile(permSCI(:,randperm(prod(nPred),100000)),1-thresh_clus));
        else % both
            thresh_clus(1) = nanmean(quantile(permSCI(:,randperm(prod(nPred),100000)),thresh_clus(1)/2));
            thresh_clus(2) = nanmean(quantile(permSCI(:,randperm(prod(nPred),100000)),1-(thresh_clus(2)/2)));
        end
    end
end
